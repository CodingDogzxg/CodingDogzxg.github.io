function funcReEnglish(text) {
    ...
    for(let i=0; i<reExp.length; i++) {
        ...
        while(res = reExp[i].exec(text)) {
            ...
            switch(i) {
                ...
                case 1 : errorClass["en-spb_err"].push(res["index"]); break;
                case 2 : errorClass["en-quo_err"].push(res["index"]); break;
                case 3 : errorClass["en-brk_err"].push(res["index"]); break;
                case 4 : errorClass["en-dsh_err"].push(res["index"]); break;
                case 5 : errorClass["en-and_err"].push(res["index"]); break;
                case 6 : errorClass["en-slh_err"].push(res["index"]); break;
                case 7 : errorClass["en-nan_err"].push(res["index"]); break;
            }
    ...
    let brk_left = 1; // left brk(1) or right brk(-1)
    for(idx of indexs) {
        if(errorClass["en-spa_err"].includes(idx)) {
            ...
            continue;
        } else if(errorClass["en-spb_err"].includes(idx)) {
            errorClass["en-spb_err"].splice(0,1);
            let solid = text.match(/^([a-z]|[A-Z]| (?![,.:;!?]])|\n|[,.:;!?] )+/g);
            let amount = !solid ? 0 : solid[0].length;
            let punc = /(?<= )[,.:;!?]/.exec(text)[0];
            newRegExp = new RegExp('(?<= )p'.replace('p', punc));
            text = text.replace(newRegExp, `<span class="alerts_corrections" onclick="show_corrections('en-spb_err');">${punc}</span>`);
            ...
            console.log(punc);
            newRegExp = new RegExp('\\p'.replace('p', punc));
            text = text.replace(newRegExp, `<span class="alerts_inspections" onclick="show_corrections('en-brk_err');">${punc}</span>`);
            answer += text.slice(0, amount + 83); // slice 不包括下标end
            text = text.slice(amount + 83);
            continue;
